<?php
include "../util/config.php";
// 각종 유틸함수
include "../util/util.php";
// MySQL 연결
$connect=my_connect($host,$dbid,$dbpass,$dbname);

$page = set_var($_POST['page']);
$lcode = set_var($_POST['lcode']);

if(!$_COOKIE['p_sid']){
  $SID = md5(uniqid(rand()));
  SetCookie("p_sid",$SID,0,"/");
}

if(!$_SESSION['p_id']){
     $id_fk = "guest";
}
else{
     $id_fk = $_SESSION['p_id'];
}

if($from == "basket"){
   if($md == "del"){
     $query = "DELETE FROM products_cart WHERE cart_id='$cart_id' ";
     mysqli_query($connect, $query);
   }else if($md == "edit"){
     $query = "UPDATE products_cart SET volume='$products_count' WHERE cart_id='$cart_id' ";
     mysqli_query($connect, $query);
   }

   // echo "<meta http-equiv='Refresh' content='0; URL=cart.php'>";
   redirect('cart.php');
} else if($from == "m_basket"){
   if($md == "del"){
     $query = "DELETE FROM products_cart WHERE cart_id='$cart_id' ";
     mysqli_query($connect, $query);

     $url = "/m/m_cart.php";
	 show_msg('삭제했습니다.', $url);
   }else if($md == "edit"){
     $query = "UPDATE products_cart SET volume='$products_count' WHERE cart_id='$cart_id' ";
     mysqli_query($connect, $query);

     $url = "/m/m_cart.php";
	 show_msg('수량을 수정했습니다.', $url);
   }

   // echo "<meta http-equiv='Refresh' content='0; URL=/m/m_cart.php'>";

} else if($from == "detail"){

	 //장바구니에 있는 상품확인
	$qry = "SELECT * FROM products_cart WHERE user_id='$id_fk' AND product_fk='$pnum' AND p_opt='$selected_opt'  ";
	$res = mysqli_query($connect, $qry);
	$row = mysqli_fetch_array($res);

	if(count($row) > 0) {
		$products_count += $row['volume'];

		$query = "UPDATE products_cart SET volume = '$products_count' WHERE cart_id='$row[cart_id]' AND user_id='$id_fk' ";
		mysqli_query($connect, $query);
	}else {
		$query = "INSERT INTO products_cart VALUES ('', '$id_fk', '$_COOKIE[p_sid]', '$pnum', '$products_count', '$amount', '$selected_opt', now() )";
		mysqli_query($connect, $query);
	}

	if($chk == "2") {
		redirect('purchase.php?from=detail');
		// echo "<meta http-equiv='Refresh' content='0; URL=purchase.php?from=detail'>";
	}else {
		// $url = "detail.php?pnum=$pnum&lcode=$lcode&mcode=$mcode&scode=$scode";
		// show_msg('장바구니에 담았습니다.', $url);

		$msg = "<meta http-equiv='content-type' content='text/html; charset=UTF-8' />\n";
        $msg .= "<script>\n";
	    $msg .= "window.alert('장바구니에 담았습니다.')\n";
	    $msg .= "</script>\n";


		// get quantity from cart
		$t_qry5 = "SELECT sum(volume) AS cnt_2 FROM products_cart WHERE user_id = '$id_fk'";
        $t_res5 = mysqli_query($connect, $t_qry5);
        $t_rows5 = mysqli_fetch_array($t_res5);
        mysqli_free_result($t_res5);

		echo json_encode(array("msg"=>$msg, "qty"=>$t_rows5['cnt_2']));

	}
} else if($from == "m_detail"){     // from mobile page

	 //장바구니에 있는 상품확인
	$qry = "SELECT * FROM products_cart WHERE user_id='$id_fk' AND product_fk='$pnum' AND p_opt='$selected_opt'  ";
	$res = mysqli_query($connect, $qry);
	$row = mysqli_fetch_array($res);

	if(count($row) > 0) {
		$products_count += $row['volume'];

		$query = "UPDATE products_cart SET volume = '$products_count' WHERE cart_id='$row[cart_id]' AND user_id='$id_fk' ";
		mysqli_query($connect, $query);
	}else {
		$query = "INSERT INTO products_cart VALUES ('', '$id_fk', '$_COOKIE[p_sid]', '$pnum', '$products_count', '$amount', '$selected_opt', now() )";
		mysqli_query($connect, $query);
	}

	if($chk == "3") {
		redirect('/m/m_purchase.php?from=m_detail');
		// echo "<meta http-equiv='Refresh' content='0; URL=/m/m_purchase.php?from=m_detail'>";
	}else {
		// $url = "/m/m_detail.php?pnum=".$pnum."&lcode=".$lcode."&mcode=".$mcode."&scode=".$scode;
		// show_msg('장바구니에 담았습니다.', $url);
		$msg = "<meta http-equiv='content-type' content='text/html; charset=UTF-8' />\n";
        $msg .= "<script>\n";
	    $msg .= "window.alert('장바구니에 담았습니다.')\n";
	    $msg .= "</script>\n";


		// get quantity from cart
		$t_qry5 = "SELECT sum(volume) AS cnt_2 FROM products_cart WHERE user_id = '$id_fk'";
        $t_res5 = mysqli_query($connect, $t_qry5);
        $t_rows5 = mysqli_fetch_array($t_res5);
        mysqli_free_result($t_res5);

		echo json_encode(array("msg"=>$msg, "qty"=>$t_rows5['cnt_2']));


	}
} else if($from == "list") {
	if(!$products_count){
	  $products_count = 1;
	}

	//장바구니에 있는 상품확인
	$qry = "SELECT * FROM products_cart WHERE user_id='$id_fk' AND product_fk='$pnum' AND p_opt='$selected_opt'  ";
	$res = mysqli_query($connect, $qry);
	$row = mysqli_fetch_array($res);

	if(count($row) > 0) {
		$products_count += $row['volume'];

		$query = "UPDATE products_cart SET volume = '$products_count' WHERE cart_id='$row[cart_id]' AND user_id='$id_fk' ";
		$result = mysqli_query($connect, $query);
	}else {
		$query = "INSERT INTO products_cart VALUES ('', '$id_fk', '$_COOKIE[p_sid]', '$pnum', '$products_count', '$amount', '$selected_opt', now() )";
		$result = mysqli_query($connect, $query);
	}


	if($chk == "2") {
		redirect('purchase.php?from=detail');
	}else {
		$msg = "<meta http-equiv='content-type' content='text/html; charset=UTF-8' />\n";
        $msg .= "<script>\n";
	    $msg .= "window.alert('장바구니에 담았습니다.')\n";
	    $msg .= "</script>\n";


		// get quantity from cart
		$t_qry5 = "SELECT sum(volume) AS cnt_2 FROM products_cart WHERE user_id = '$id_fk'";
        $t_res5 = mysqli_query($connect, $t_qry5);
        $t_rows5 = mysqli_fetch_array($t_res5);
        // mysqli_free_result($t_res5);

		echo json_encode(array("msg"=>$msg, "qty"=>$t_rows5['cnt_2']));
	}


} else if($from == "new") {
	if(!$products_count){
	  $products_count = 1;
	}

    //장바구니에 있는 상품확인
	$qry = "SELECT * FROM products_cart WHERE user_id='$id_fk' AND product_fk='$pnum' AND p_opt='$selected_opt'  ";
	$res = mysqli_query($connect, $qry);
	$row = mysqli_fetch_array($res);

	if(count($row) > 0) {
		$products_count += $row['volume'];

		$query = "UPDATE products_cart SET volume = '$products_count' WHERE cart_id='$row[cart_id]' AND user_id='$id_fk' ";
		mysqli_query($connect, $query);
	}else {
		$query = "INSERT INTO products_cart VALUES ('',	'$id_fk','$_COOKIE[p_sid]', '$pnum', '$products_count',	'$amount', '$selected_opt', now() )";
		mysqli_query($connect, $query);
	}

	if($chk == "2") {
		redirect('purchase.php?from=detail');
		// echo "<meta http-equiv='Refresh' content='0; URL=purchase.php?from=detail'>";
	}else {
		$url = "sub_list.php?mode=$mode&page=$page";
		show_msg('장바구니에 담았습니다.', $url);
	}
} else if($from == "dir") { // 비회원 바로구매
    //장바구니에 있는 상품확인
	$qry = "SELECT * FROM products_cart WHERE user_id='$id_fk' AND product_fk='$pnum' AND p_opt='$selected_opt'  ";
	$res = mysqli_query($connect, $qry);
	$row = mysqli_fetch_array($res);

	if(count($row) > 0) {
		$products_count += $row['volume'];

		$query = "UPDATE products_cart SET volume = '$products_count' WHERE cart_id='$row[cart_id]' AND user_id='$id_fk' ";
		mysqli_query($connect, $query);
	}else {
		$query = "INSERT INTO products_cart VALUES ('', '$id_fk', '$_COOKIE[p_sid]', '$pnum', '$products_count', '$amount', '$selected_opt', now() )";
		mysqli_query($connect, $query);
	}
	redirect('purchase_p.php?from=dir');
 	// echo "<meta http-equiv='Refresh' content='0; URL=purchase_p.php?from=dir'>"; //별도의 구매페이지로 이동
} else if($chk == "5") { // 검색결과
    //장바구니에 있는 상품확인
	$qry = "SELECT * FROM products_cart WHERE user_id='$id_fk' AND product_fk='$pnum' AND p_opt='$selected_opt'  ";
	$res = mysqli_query($connect, $qry);
	$row = mysqli_fetch_array($res);

	if(count($row) > 0) {
		$products_count += $row['volume'];

		$query = "UPDATE products_cart SET volume = '$products_count' WHERE cart_id='$row[cart_id]' AND user_id='$id_fk' ";
		mysqli_query($connect, $query);
	}else {
		$query = "INSERT INTO products_cart VALUES ('', '$id_fk', '$_COOKIE[p_sid]', '$pnum', '$products_count', '$amount', '$selected_opt', now() )";
		mysqli_query($connect, $query);
	}

	$url = "sub_list.php?mode=search&key=$key&keyword=$keyword&page=$page";
	show_msg('장바구니에 담았습니다.', $url);
} else {
	redirect('cart.php');
 	// echo "<meta http-equiv='Refresh' content='0; URL=cart.php'>";
}
?>
