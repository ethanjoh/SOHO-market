...
### 브랜드 및 사명 명칭 업데이트 (Brand Name & Company Name Update)
- **신수상사 및 신수마켓 명칭 일괄 교체**
  - 소스 코드 및 이메일 템플릿 파일 내에서 사용되던 기존 브랜드명/사명을 새로운 명칭으로 일괄 교체 완료:
    - `(주)신수트레이딩`, `신수트레이딩`, `신수마켓` -> `소호마켓`
    - `신수상사`, `신수` -> `마켓`
    - (단, 이메일 주소 및 외부 링크 도메인의 영문 식별값 `shinsoo`와 PSD 바이너리, 업로드 데이터 내역 등은 제외)
  - 기존 백업 파일인 `include/header_org.php` 삭제 완료
