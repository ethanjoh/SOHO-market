...
?>
      <form name="form1" method="post" action="mem_sendmail_list.php">
	...
	$sql_2 = "select * from member where md_email <> '' $sear_char order by seq_num desc LIMIT $cline,$scale1 "; 
 ...
 ?>
              <td><?=$bunho?></td>
              <td><a href="javascript:open_win('mem_view_member.php?num=<?=$list['seq_num']?>&amp;from=mail','nwin','scrollbars=yes,resizable=yes, width=650,height=650');">
                <?=$list['id']?>
                </a></td>
        ...
        <table summary="send mail">
          <tbody>
            <tr>
              ...
              <td width="45%"><?php
	 $url = "mem_sendmail_list.php?$id=$id&mode=$mode&license_no=$license_no&md_email=$md_email&o_phone=$o_phone&company_name=$company_name&page_scale=$page_scale"; 
