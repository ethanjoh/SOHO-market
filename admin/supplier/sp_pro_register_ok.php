...
*/  
  ...
  if($_FILES['d_image']['name']){  
  	$file8 = $savedir."/b/".substr(md5(uniqid($g4[server_time])),0,8)."_".$_FILES['d_image']['name'];
   ...
   $opt = addslashes($opt);
   $opt_stock = addslashes($opt_stock);   
   ...
   $dbinsert1 = "INSERT INTO products(prod_code, category_l, category_m, category_s,
										name, company, id, origin, retail_price, sale_price, fixed_price,
										mileage, moq, opt, opt_stock, size, material, stock, con_html, contents,
										...
										created, option1_chk, option2_chk, del_chk, event, date1, date2)
			    		VALUES('$trade_code', '$lcode', '$mcode', '$scode',
					  		 		'$name', '$company', 'admin', '$origin', '$retail_price', '$sale_price', '$fixed_price',
				      		 		'$mileage', '$moq', '$opt', '$opt_stock', '$size', '$material', '$stock', '$con_html', '$contents',
					  		 		'$simg_chk',   '$file1',
					   		 		'$bimg1_chk', '$file3',
							 		'$bimg2_chk', '$file4',
							 		...
							 		'$bimg5_chk', '$file7',
							 		'$dimg_chk',  '$file8',
							 		now(), '$option1_chk', '$option2_chk', '$del_chk', '$event', '$date1', '$date2')";
   ...
   if($result1){    
	  ...
	  ");
      echo "<meta http-equiv='Refresh' content='0; URL=sp_products_list.php?lcode=$lcode&mcode=$mcode&scode=$scode'>"; 
    } else{
...
/*        
  if($_FILES['s_image']['name']){
   	$temp1 = $savedir."/s/".substr(md5(uniqid($g4[server_time])),0,8)."_".$_FILES['s_image']['name'];
  	move_uploaded_file($_FILES['s_image']['tmp_name'], $temp1);	
...
*/  
  ...
  if($_FILES['d_image']['name']){
   	$temp8 = $savedir."/b/".substr(md5(uniqid($g4[server_time])),0,8)."_".$_FILES['d_image']['name'];
  ...
  if($con_html=='2'){
   ...
   $dbinsert1 = "UPDATE products SET category_l='$lcode',
	...
	$query1 = "SELECT * FROM products WHERE num=$p_num";
 	...
 	$sql = "INSERT INTO products(prod_code, category_l, category_m, category_s,
									...
									mileage, moq, opt, opt_stock, size, material, stock, con_html, contents,
									created, option1_chk, option2_chk, del_chk, event, date1, date2)
	VALUES('$trade_code', '$row1[category_l]', '$row1[category_m]', '$row1[category_s]',
					  		 		'$row1[name]', '$row1[company]', '$row1[id]', '$row1[origin]', '$row1[retail_price]', '$row1[sale_price]', '$row1[fixed_price]',
				      		 		'$row1[mileage]', '$row1[moq]', '$row1[opt]', '$row1[opt_stock]', '$row1[size]', '$row1[material]', '$row1[stock]', '$row1[con_html]', '$row1[contents]',
							 		now(), '$row1[option1_chk]', '$row1[option2_chk]', '$row1[del_chk]', '$row1[event]', '$row1[date1]', '$row1[date2]')";
   ...
   if($result2){    
		echo "<meta http-equiv='content-type' content='text/html; charset=UTF-8' />
      	...
      	echo "<meta http-equiv='Refresh' content='0; URL=sp_pro_register.php?mode=update&prod_code=$trade_code&lcode=$row1[category_l]&mcode=$row1[category_m]&scode=$row1[category_s]'>"; 
    } else{
