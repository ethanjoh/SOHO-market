				...
				<div class="row">
					...
					switch ($mode) {
						...
						case 'search' : $sql_2 = "SELECT * FROM mall_order
					                          	  WHERE user_id <> 'guest' AND (buyer_name LIKE '%$key_value%' OR user_id LIKE '%$key_value%' OR recipient_name LIKE '%$key_value%' OR goods_name LIKE '%$key_value%')"; break;

						case 'date'   : $sql_2 = "SELECT orderid FROM mall_order
						                          WHERE cancel = 'N' AND date(createdate) BETWEEN '$date1' AND '$date2' AND user_id = '$key_value' "; break;
						case 'today'  : $today = date("Y-m-d");
						                $sql_2 = "SELECT orderid FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND date(createdate) = '$today' ";	break;
						case 'unchk'  : $sql_2 = "SELECT orderid FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '3' "; break;
						case 'chk'    : $sql_2 = "SELECT orderid FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '5' "; break;
						case 'paid'   : $sql_2 = "SELECT orderid FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '7' "; break;
						case 'nopaid' : $sql_2 = "SELECT orderid FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND pchk = 'N' "; break;
						case 'delay'  : $sql_2 = "SELECT orderid FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '0' "; break;
						case 'finish' : $sql_2 = "SELECT orderid FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '8' "; break;
						case 'cancel' : $sql_2 = "SELECT orderid FROM mall_order
						                          WHERE cancel = 'Y' AND user_id <> 'guest' "; break;
						case 'return' : $sql_2 = "SELECT * FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '-1' "; break;
		...
		<?php   } ?>
				...
				<div class="row">
					...
					switch ($mode) {
						...
						case 'search' : $sql_4 = "SELECT * FROM mall_order
						                          WHERE user_id <> 'guest' AND (buyer_name LIKE '%$key_value%' OR user_id LIKE '%$key_value%' OR recipient_name LIKE '%$key_value%' OR goods_name LIKE '%$key_value%') ORDER BY num DESC LIMIT $cline,$scale1 "; break;

						case 'date'   : $sql_4 = "SELECT * FROM mall_order
						                          WHERE cancel = 'N' AND date(createdate) BETWEEN '$date1' AND '$date2' AND user_id = '$key_value' ORDER BY num DESC LIMIT $cline,$scale1 "; break;
						case 'today'  : $today = date("Y-m-d");
						                $sql_4 = "SELECT * FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND date(createdate) = '$today' ORDER BY num DESC LIMIT $cline,$scale1 "; break;
						case 'unchk'  : $sql_4 = "SELECT * FROM mall_order
						                          WHERE cancel = 'N' AND status = '3' AND user_id <> 'guest' ORDER BY num DESC LIMIT $cline,$scale1 "; break;
						case 'chk'    : $sql_4 = "SELECT * FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '5' ORDER BY num DESC LIMIT $cline,$scale1 ";	break;
						case 'paid'   : $sql_4 = "SELECT * FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '7' ORDER BY num DESC LIMIT $cline,$scale1 ";	break;
						case 'nopaid' : $sql_4 = "SELECT * FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND pchk = 'N' ORDER BY num DESC LIMIT $cline,$scale1 "; break;
						case 'delay'  : $sql_4 = "SELECT * FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '0' ORDER BY num DESC LIMIT $cline,$scale1 ";	break;
						case 'finish' : $sql_4 = "SELECT * FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '8' ORDER BY num DESC LIMIT $cline,$scale1 ";	break;
						case 'cancel' : $sql_4 = "SELECT * FROM mall_order
						                          WHERE cancel = 'Y' AND user_id <> 'guest' ORDER BY num DESC LIMIT $cline,$scale1 "; break;
						case 'return' : $sql_4 = "SELECT * FROM mall_order
						                          WHERE cancel = 'N' AND user_id <> 'guest' AND status = '-1' ORDER BY num DESC LIMIT $cline,$scale1 "; break;
