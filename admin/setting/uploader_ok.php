...
$uploaddir = '../../upload/p_image/d/'; //서버에 저장될 디렉토리의 권한은 777로 해둔다.
...
if($_FILES['uploadedfile']['name'] != "") {
		...
		$result = move_uploaded_file($_FILES["uploadedfile"]["tmp_name"], $filename);
		//chmod('$filename',0707); // 파일에 권한 설정
		if($result) {
			//절대경로로 바꿈
			$tmp = explode('/', $filename);
			$filename = $tmp[sizeof($tmp)-4]."/".$tmp[sizeof($tmp)-3]."/".$tmp[sizeof($tmp)-2]."/".$tmp[sizeof($tmp)-1];
?>
