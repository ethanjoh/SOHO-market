...
$qry1 = "SELECT count(*) as cnt FROM products p, products_cart c WHERE c.user_id='test' AND p.num=c.product_code";
$res1 = mysqli_query($connect, $qry1);
$crow = mysqli_fetch_array($res1);

// echo "<pre>";
// print_r($crow['cnt']);
// echo "</pre>";

// for ($k = 0; $k < $crow['cnt']; $k++) {
...
for ($i = 0; $rows = mysqli_fetch_array($result); $i++) {
    ...
    $qry2 = "SELECT * FROM products p, products_cart c WHERE c.user_id='mc4520' AND p.num=c.product_code";
    $res2 = mysqli_query($connect, $qry2);
    $row2 = mysqli_fetch_array($res2);

    $final_opt_count[$i] = explode(",", $row2['opt_count']); //초기화
    $final_opt_stock[$i] = explode(",", $row2['opt_stock']); //초기화
    ...
    for ($j = 0; $j < sizeof($products_opt); $j++) {
        ...
        if ($products_opt[$j] == $order_opt[$i]) {
            ...
            // 전체재고에서 주문수량 차감
            $final_opt_count[$i][$j] = $products_opt_count[$j] - $order_count[$i];
